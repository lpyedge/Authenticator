...
class BitMatrix {
  ...
  #offset(x, y) {
    return y * this.#rowSize + utils.toInt32(x / 32);
  }
  /**
   * @property width
   ...
   */
  set(x, y) {
    this.#bits[this.#offset(x, y)] |= utils.getBitMask(x);
  }
  /**
   * @method get
   ...
   */
  get(x, y) {
    return utils.toBit(this.#bits[this.#offset(x, y)] >>> utils.getBitOffset(x));
  }
  flip(x, y) {
    if (x != null && y != null) {
      this.#bits[this.#offset(x, y)] ^= utils.getBitMask(x);
  ...
  setRegion(left, top, width, height) {
    ...
    for (let y = top; y < bottom; y++) {
      ...
      for (let x = left; x < right; x++) {
        bits[offset + utils.toInt32(x / 32)] |= utils.getBitMask(x);
      }
    }
  }
}
