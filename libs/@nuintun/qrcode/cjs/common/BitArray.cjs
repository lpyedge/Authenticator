...
class BitArray {
  ...
  set(index) {
    this.#bits[offset(index)] |= utils.getBitMask(index);
  }
  get(index) {
    return utils.toBit(this.#bits[offset(index)] >>> utils.getBitOffset(index));
  }
  xor(mask) {
    const bits = this.#bits;
